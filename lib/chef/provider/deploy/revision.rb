    ...
    class Deploy
      class Revision < Chef::Provider::Deploy
        ...
        def cleanup!
          ...
          PathHelper.glob(PathHelper.escape_glob(new_resource.deploy_to), "releases", "*").each do |release_dir|
        ...
        def sorted_releases_from_filesystem
          PathHelper.glob(PathHelper.escape_glob(new_resource.deploy_to), "releases", "*").sort_by { |d| ::File.ctime(d) }
        end
